...
function showButton(...buttonsToShow) {
  const buttonsToHide = allButtons.filter(
    (btn) => !buttonsToShow.includes(btn)
  );
...
class Timer {
  ...
  start() {
    if (typeof window.isAnyTaskRunning === "function" && !this.isRunning) {
      if (window.isAnyTaskRunning()) {
        alert(
          "Another task is already running. Please pause or complete it before starting a new one."
        );
        return;
      }
    }
    ...
    if (this.type === TIMER_TYPE.POMODORO) {
      ...
      this.currentBlockStartTimestamp =
        Date.now() - (block.duration - this.pomoBlockRemaining) * 1000;
    } else {
  ...
  pause() {
    ...
    if (this._elapsedStartTime) {
      const elapsedNow = Math.floor(
        (Date.now() - this._elapsedStartTime) / 1000
      );
  ...
  _runTimer() {
    ...
    if (this.type === TIMER_TYPE.POMODORO) {
      this.interval = setInterval(() => {
        ...
        const timeInBlock = Math.floor(
          (Date.now() - this.currentBlockStartTimestamp) / 1000
        );
        ...
        if (typeof this.onTickXP === "function") {
          const now = Date.now();
          const secondsSinceLastXP = Math.floor(
            (now - this.lastXPCheck) / 1000
          );
  ...
  getPomodoroBlockType() {
    ...
    const totalElapsed = Math.floor(
      (Date.now() - this.pomodoroStartTimestamp) / 1000
    );
    let timePassed = 0;
  ...
  restorePomodoroState(state) {
    ...
    const secondsUsed =
      ...
      (state.pomoBlockRemaining ?? currentBlock.duration);
  ...
  static updateTimerStat(blockType) {
    ...
    else if (blockType === "shortBreak")
      timerStatEl.textContent = "Short Break";
