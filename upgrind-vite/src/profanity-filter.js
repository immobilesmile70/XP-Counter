...
const normalizedBannedWords = LeoProfanity.getDictionary().map((w) =>
  normalizeLeetspeak(w)
);
...
export const filter = {
  ...
  stich(text) {
    return text.replace(/[^a-z\u0900-\u097F\u0600-\u06FF]/gi, "");
  },
  ...
  hasSocialHandle(text) {
    const socialRegex =
      /(?:@|https?:\/\/)?(?:instagram\.com|twitter\.com|t\.me|facebook\.com|snapchat\.com|linkedin\.com|@)[\w.]+/gi;
  ...
  hasFuzzyMatch(text) {
    ...
    return (
      words.some(checkWord) || checkWord(normalizedText.replace(/\s+/g, ""))
    );
  },
    ...
    return cleanText
      ...
      .map((part) => {
        if (/[a-zA-Z0-9\u0600-\u06FF\u0900-\u097F]+/.test(part)) {
          return this.isProfane(part) ? "*".repeat(part.length) : part;
        }
        return part;
      })
      .join("");
