...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const emailInputForgetPass = document.getElementById(
    "email-input-forget-pass"
  );
    ...
    try {
      ...
      if (now - lastChange < TEN_DAYS_MS) {
        const daysLeft = Math.ceil(
          (TEN_DAYS_MS - (now - lastChange)) / (24 * 60 * 60 * 1000)
        );
        clearTimeout(popupTimeout);
        isInfoPopup = true;
      ...
      const usernameTaken = Object.values(usersData).some(
      ...
      await set(
        ref(database, `users/${auth.currentUser.uid}/username`),
        newUsername
      );
      await set(
        ref(database, `users/${auth.currentUser.uid}/lastUsernameChange`),
        now
      );
  ...
  const refreshLeaderboardButton = document.getElementById(
    "refresh-leaderboard"
  );
  ...
  signUpWithGoogleButton.addEventListener("click", () => {
    showDialog(
      "For your information",
      'Please accept the <a style="color: var(--subtext);" href="/terms">Terms and Conditions</a> and <a style="color: var(--subtext);" href="/privacy">Privacy Policy</a> before signing up.',
      [
        {
          text: "OK",
          onClick: async () => {
  ...
  logOutButton.addEventListener("click", () => {
    showDialog(
      ...
      'Logging out will <span style="color: red;">remove your local data and settings</span>, but will keep your cloud data like tasks, xp, etc. You can log back in anytime to restore them.',
      [
        {
          text: "Yes",
          onClick: () => {
  ...
  deleteUserButton.addEventListener("click", () => {
    ...
    showDialog(
      ...
      'Deleting your account will <span style="color: red;">erase all your data and settings permanently.</span> You won’t be able to recover them, even if you log in again. You can read the <a style="color: var(--subtext);" href="/privacy">Privacy Policy</a> for more information about how your data is handled after deletion.',
      [
        {
          text: "Yes",
          onClick: () => {
            if (user.providerData[0].providerId === "password") {
              ...
              setTimeout(() => {
                showDialog(
                  ...
                  'To delete your account, please re-enter your password to confirm your identity. <span style="color: blue;">This ensures your account is deleted securely and intentionally.</span>',
                  [
                    {
                      text: "Delete",
                      onClick: async () => {
              ...
              setTimeout(() => {
                showDialog(
                  ...
                  'To delete your account, please re-authenticate using your Google account. <span style="color: blue;">This step is required to verify your identity and complete the deletion process securely.</span>',
                  [
                    {
                      text: "Delete",
                      onClick: async () => {
              ...
              get(ref(database, "users"))
                .then(async (snapshot) => {
                  ...
                  if (filter.isProfane(username)) {
                    ...
                    isInfoPopup = false;
                    showPopup(
                      "Username contains profanity. Please choose another one."
                    );
                    return;
                  } else {
                    justSignedUp = true;
    ...
    if (email) {
      sendPasswordResetEmail(auth, email)
        .then(async () => {
          ...
          clearTimeout(popupTimeout);
          showPopup(
            `Recovery email sent to ${email}. Please check your inbox.`
          );
          loginForm.classList.remove("hidden");
          forgetPassForm.classList.add("hidden");
        })
