...
function getDurationInSeconds(
  ...
  if (typeof minutesInput === "string")
    minutesInput = document.getElementById(minutesInput);
  if (typeof secondsInput === "string")
    secondsInput = document.getElementById(secondsInput);
  if (hoursInput && hoursInput.value != null)
    h = parseInt(hoursInput.value) || 0;
  if (minutesInput && minutesInput.value != null)
    m = parseInt(minutesInput.value) || 0;
  if (secondsInput && secondsInput.value != null)
...
function queueFirebaseUpdate(task, changes = {}) {
  ...
  firebaseFlushQueue[task.id] = setTimeout(() => {
    ...
    if (
      window.taskManager &&
      typeof window.taskManager.getCurrentTask === "function"
    ) {
      const current = window.taskManager.getCurrentTask();
      if (
        current &&
        current.id === task.id &&
        typeof window.taskManager.setCurrentTask === "function"
      ) {
...
async function createTask() {
  ...
  const shortBreakDuration = getDurationInSeconds(
    shortBreakHoursInput,
    shortBreakMinutesInput,
    shortBreakSecondsInput,
    true
  );
  const longBreakDuration = getDurationInSeconds(
    longBreakHoursInput,
    longBreakMinutesInput,
    longBreakSecondsInput,
    true
  );
  if (type === "count_down" && duration <= 0) {
    showPopupWithType(
      "Please enter a valid duration for your countdown task.",
      false
    );
    return;
  }
  if (type === "pomodoro") {
    if (duration <= 0) {
      showPopupWithType(
        "Please enter a valid work duration for your Pomodoro task.",
        false
      );
      return;
    }
    if (shortBreakDuration <= 0) {
      showPopupWithType(
        "Please enter a valid short break duration for your Pomodoro task.",
        false
      );
      return;
    }
    if (longBreakDuration <= 0) {
      showPopupWithType(
        "Please enter a valid long break duration for your Pomodoro task.",
        false
      );
      return;
    }
  ...
  if (
    window.taskManager &&
    typeof window.taskManager.getAllTasks === "function"
  ) {
    const allTasks = window.taskManager.getAllTasks();
    if (allTasks.length >= taskLimit) {
      showDialog(
        "For your information",
        "You can only have up to 8 tasks saved. <strong>Please delete a task before adding a new one.</strong>",
        [
          {
            text: "OK",
            onClick: () => {
  ...
  const task = window.taskManager.createTask({
    name,
    type,
    duration,
    shortBreakDuration,
...
function refreshTaskList() {
  ...
  taskListEl.innerHTML =
    '<span id="no-tasks-at-all" style="display: none;">You don\'t have any tasks right now. Try Creating one!</span>';
  ...
  tasks.forEach((task, idx) => {
    ...
    if (task.type === "count_up") {
      const elapsed =
        typeof task.elapsedTime === "number" && task.elapsedTime > 0
          ? task.elapsedTime
          : 0;
    ...
    div.querySelector(".start-task").onclick = (e) => {
      ...
      if (mainSection && tasksSection && createTaskSection) {
        ...
        setTimeout(() => {
          tasksSection.classList.add("hide");
        }, 350);
        createTaskSection.classList.add("hidden");
        setTimeout(() => {
          createTaskSection.classList.add("hide");
        }, 350);
      }
    ...
    div.querySelector(".delete-task").onclick = (e) => {
      ...
      showDialog(
        ...
        'Once deleted, it can\'t be restored as <span style="color: red;">this action cannot be undone from our side.</span>',
        [
          {
            text: "Yes",
            onClick: () => {
    ...
    div.querySelector(".complete-task").onclick = (e) => {
      ...
      showDialog(
        "Do you really want to mark the task as complete?",
        'Once marked, it will be deleted and can\'t be restored as <span style="color: red;">this action cannot be undone from our side.</span>',
        [
          {
            text: "Yes",
            onClick: () => {
...
function startTask(taskId) {
  ...
  activeTaskId = taskId;
  let timerType =
    task.type === "pomo" || task.type === "pomodoro" ? "pomodoro" : task.type;
  ...
  if (activeTimer) {
    ...
    if (task) {
      ...
      queueFirebaseUpdate(task, {
        ...
        pomodoroState: task.pomodoroState,
      });
    }
  }
}
...
function resetTask() {
  showDialog(
    ...
    'Once you reset the timer, all of your current progress realted to this task will be reset. <span style="color: red;">This action cannot be undone.</span>',
    [
      {
        text: "Yes",
        onClick: () => {
          if (activeTimer) {
            ...
            if (stopwatchEl) {
              if (activeTimer.type === "pomodoro") {
                stopwatchEl.textContent = Timer.formatTime(
                  activeTimer.getCurrentTime()
                );
                Timer.updateTimerStat(activeTimer.getPomodoroBlockType());
              } else {
                stopwatchEl.textContent = Timer.formatTime(
...
function bindTaskFormEvents() {
  ...
  if (createTaskStartButton)
    createTaskStartButton.onclick = async () => {
      await createTask();
      const currentTask =
        window.taskManager &&
        typeof window.taskManager.getCurrentTask === "function"
...
function bindGlobalUIEvents() {
  if (startBtn)
    startBtn.onclick = () => {
      if (activeTaskId) startTask(activeTaskId);
    };
...
function loadEditValues(taskId) {
  ...
  if (counterTypeSelect) {
    ...
    if (parent) {
      ...
      if (selectedDiv && itemsDiv) {
        const option = Array.from(counterTypeSelect.options).find(
          (opt) => opt.value === typeVal
        );
        if (option) {
          ...
          const item = Array.from(
            itemsDiv.querySelectorAll(".select-item")
          ).find((item) => item.textContent === option.text);
          if (item) item.classList.add("selected");
        }
      }
    }
    if (typeof updateDurationBreakFields === "function")
  ...
  if (task.type === "pomodoro" || task.type === "pomo") {
    setDurationInputs(
      task.shortBreakDuration,
      shortBreakHoursInput,
      shortBreakMinutesInput,
      shortBreakSecondsInput
    );
    setDurationInputs(
      task.longBreakDuration,
      longBreakHoursInput,
      longBreakMinutesInput,
      longBreakSecondsInput
    );
...
if (editTaskButton) {
  editTaskButton.onclick = async function () {
    ...
    let newDuration = getDurationInSeconds(
      durationHoursInput,
      durationMinutesInput,
      durationSecondsInput
    );
    let newShortBreak = getDurationInSeconds(
      shortBreakHoursInput,
      shortBreakMinutesInput,
      shortBreakSecondsInput,
      true
    );
    let newLongBreak = getDurationInSeconds(
      longBreakHoursInput,
      longBreakMinutesInput,
      longBreakSecondsInput,
      true
    ...
    if (newType === "count_down" && task.duration !== newDuration)
      changed = true;
    if (
      newType === "pomodoro" &&
...
function resetTaskForm() {
  ...
  if (counterTypeSelect) {
    ...
    if (parent) {
      ...
      const itemsDiv = parent.querySelector(".select-items");
      if (selectedDiv && itemsDiv) {
        const upOption = Array.from(counterTypeSelect.options).find(
          (opt) => opt.value === "up"
        );
        if (upOption) {
          ...
          itemsDiv
            .querySelectorAll(".select-item")
            .forEach((item) => item.classList.remove("selected"));
          const upItem = Array.from(
            itemsDiv.querySelectorAll(".select-item")
          ).find((item) => item.textContent === upOption.text);
